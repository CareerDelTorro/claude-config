...
160. **A FAN-OUT IS FOR WORK I CANNOT DO, NOT WORK I DO NOT FEEL LIKE DOING.** ~35M subagent tokens on a docs consolidation, until the user said "that's using to many tokens". The first fan-out was fair - 38 docs genuinely needed parallel reading. The three verification panels and the code harvest after it were READING I could do myself, cheaper and more accurately: twice a panel's headline was confidently wrong (a statistic quoted from a month-stale report; "three files were never restored" when a one-line diff showed all of them present), so I hand-verified it anyway - paying twice and trusting the expensive answer less than my own. **A fan-out feels like rigour because it is BIG, and bigness has no relationship to correctness.** Delegated reading also arrives laundered: an agent's summary reads as authoritative in a way my own skim does not, which is backwards - I can check my own reading and cannot check theirs without redoing it. Before spawning, ask what the fleet does that I cannot: more files than fit my context, genuinely independent work, or an adversarial check where MY bias is the risk. Not: I have the files, I know where to look, and I want to look thorough. Tells: fanning out over files I have already read; the brief is "read X and tell me what it says"; I could not act on the answer without opening the file myself. **When a verification pass has to be verified, it is not one** - kill it and read the thing. And the user pays per token and cannot see the bill until it lands.
