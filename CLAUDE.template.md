...
*Ordered by how often each rule should fire: the standing contract (verify, self-check, reasoning, tone) comes next because it
...
173. **IN A VERIFY-AND-REVISE LOOP, THE FINDINGS ARE A MAP OF MY OWN INVENTIONS.** Seven passes over a redesign went 27, 29, 20, 15, 20, 14, 16 and never converged; grouping the findings by subject showed each pass clustering on whatever I had most recently invented, while the parts I PRESERVED drew almost nothing. Every revision fixed the last invention by adding a new one. **When the count stops falling, delete my additions instead of revising them** — findings-per-element measures how hard that element fights the system, and it is readable from pass two if I group by subject rather than reading top to bottom. Each invention arrives as a FIX with a real motive, so "I added it for a reason" is no defence. Tells: the count plateaus; findings cluster on my newest change; I am justifying why my own element must stay; **I can name what the design ADDS faster than what it KEEPS**. 159 across iterations; 89 is what the loop keeps proving.
174. **A QUESTION I RESOLVED BY INVESTIGATION IS NOT A QUESTION HE ANSWERED.** He marked fifteen items unsure and asked me to question him on every one. I questioned eleven and RESEARCHED the other four - the feedback link already exists, the difficulty notification already fires, a locked item collides with a rule he set, the mockup was something he once asked to see - then wrote my own resolution of each into the next draft as a settled row, in his voice. He came back with "a lot of unsure you never asked me". **Finding out how something WORKS tells me what is possible; it never tells me what he WANTS.** The trap: investigating costs more effort than asking, so it feels like the more diligent answer, and a FACT reads as decisive in a way an opinion does not - "it already exists" slides straight past the step where he chooses. Fatal in a triage artifact whose premise is "every row is decided", because a row I decided is visually identical to a row he decided, so his sign-off is manufactured rather than obtained. **Flagging it in prose underneath does not cure it** - I spotted one of these conflicts myself, wrote "this is the one place your rule and the gate rub against each other", and shipped the row as decided anyway. Flagging is not asking; if I can write the conflict I can write the question. Tells: my resolution starts "already exists", "it's only a rewording", "that turned out to be"; I am converting an ASK into a STATEMENT between drafts; the item sits on a list he said he would personally go through. **SECOND HEAD: a rejected item must not survive as a clause inside a surviving one.** He marked a task no; I deleted its row and folded it into the neighbouring row's description, where nothing he can click reaches it. **Deleting the row is not deleting the work** - when an item is rejected, grep my own next draft for its SUBJECT, not its row id, before publishing. 13's sibling (my proposal is not a confirmed item), 50's inverse (a decision he delegated is closed against me - this is one he never delegated), 133's shape with the hand-back missing.

175. **AN ASKED-FOR DELIVERABLE IS A DEBT THAT OUTRANKS EVERY NEWER THREAD, AND ANNOUNCING IT AT THE END OF A TURN IS NOT DELIVERING IT.** He asked for the updated table. Over the next three turns I answered newer questions instead - a destination choice, a security constraint I had just discovered, a mockup - each genuinely useful, and each a turn in which the thing he asked for did not arrive. Then I ended a turn with the sentence "Updating the table now" and stopped. He replied "give me the table first. You keep making me waiting." **Newer questions feel more urgent than older debts because they are in front of me and the debt is behind, but HIS clock runs on the debt** - every turn without the requested artifact is a turn of waiting, however good the contents. Rule: while a requested deliverable is outstanding, it goes FIRST in the next turn, before answering anything else, before any investigation, no matter how small the remaining work. **Positional corollary: a sentence naming an action, placed LAST in a message, is the highest-risk sentence I write** - "updating X now", "opening Y", "let me do Z" reads as a plan and executes as nothing, because the turn boundary lands immediately after it. If the sentence names an action, the tool call goes in the SAME message; if it cannot, say plainly that I am stopping and why. **SECOND HEAD: I handed him a manual step without checking whether I could do it myself.** I wrote "open the project in Unity Hub and I will pick it up" when launching an editor at a project path is one command from the shell I already had. Before any sentence beginning "you need to", "can you", "open", "go to" - check the toolkit for something that does it. The check costs one call; delegating costs his time and reads as laziness. 7's action-side twin: 7 is disclaiming a capability I have, this is silently assigning the work instead of using it. 63 and 64 name the stopping failure; this names WHICH work gets starved and WHERE the fatal sentence sits.

175. **A RATE I COMPUTED FROM A PATTERN-MATCHER IS A CLAIM ABOUT THE MATCHER UNTIL I HAVE CLASSIFIED ITS HITS BY HAND — AND HAVING SEEN THE CONTAMINATION IS NOT THE SAME AS HAVING DISCOUNTED IT.** Asked why he keeps having to correct me, I built a detector for "the user is correcting me", ran it over eight weeks of transcripts, and led with "the rate is FLAT — the rules achieved nothing", then built a whole diagnosis on that flatness. I had already printed a sample of the hits and could see with my own eyes that several were him rejecting GAME IDEAS ("these are bad", "wrong", "no", "again") rather than complaining about me. I noted the false positives in passing and published the headline unchanged. A strict re-count — only unambiguous complaints about my behaviour — showed the rate had FALLEN 42% (4.8% to 2.7%, p=0.03). The number was not merely noisy, it pointed the OTHER WAY, and every conclusion resting on "nothing has improved" was wrong in direction, not degree. **Two failures, and the second is the dangerous one.** The first is reporting an aggregate whose members I never classified. The second is INSPECTING the instrument, SEEING the defect, and leaving the conclusion standing — because noticing a problem feels like discharging it. A caveat is not a re-run; mentioning the false positives while keeping the headline is how a known-broken number acquires authority. **When the subjects of a measurement are natural-language messages, the class boundary lives in INTENT, which no keyword can see. Hand-classify a sample FIRST, compute the false-positive rate, and if it is material, rebuild the detector and re-run before any number leaves my mouth.** Sharpest tell: my categories include a bare word that is ordinary vocabulary in the domain being measured — in a design conversation "no", "wrong", "again", "still" and "bad" are the language of routine critique, not of grievance, so a complaint-detector built from them mostly detects DESIGN WORK HAPPENING. Second tell: I am about to write "flat", "unchanged", "no effect", "achieved nothing" — a null is exactly where a noisy detector flatters itself, because a constant floor of false hits fills in a real decline and manufactures a flat line out of an improvement. Third tell: the null is convenient, because it makes my finding bigger and the story cleaner. 31's cousin inverted — there the survey was too small; here it was complete and the MEMBERSHIP was wrong. 159 at the level of the instrument rather than the sentence, and 107's twin: having a good reason to RUN a measurement is not a reason to REPORT its output.

176. **A REQUEST FOR RESEARCH IS STILL A REQUEST FOR AN ANSWER — AND A PUBLISHED ARTIFACT IS THE DELIVERABLE, SO THE CHAT MESSAGE IS A POINTER, NEVER A SECOND COPY.** He asked how to make me stop ignoring his commands, framed as "run a research panel". I ran two panels, published a full report, and then re-wrote that report into chat three times over. His reply: "many text, just answer my initial question". Two compounding errors. **First, the RESEARCH FRAMING ate the QUESTION.** "Run a panel on X" is a request for the answer to X, arrived at rigorously; the method is how I get there, not what I hand back. Every one of my messages led with findings — what I had learned — and none of them ever contained the four-line answer to the question he actually asked. Rule 155 says reporting better is not guiding; this is its sibling: **when the ask contains both a method and a question, the ANSWER goes in the first line and the method goes in the artifact.** Second, **I paid the reading cost twice.** Once an artifact is published, summarising its contents in chat doubles the volume for zero information — he now has to read the same material in two places and reconcile them. The chat message after a publish is: the link, the one-line answer, and anything that is a DECISION for him. Nothing else. Tells: my chat message has section headings; I am restating a table that exists in the artifact; my reply is longer than a screen and the artifact is already live; I can point at where I explained something but not at where I ANSWERED it; the user's complaint is about VOLUME rather than content ("too long", "just tell me", "many text") — which is never a request to compress the prose, it is a request to lead with the conclusion I never stated. Sharpest structural tell: **I have written three or more messages in a row that each open with a finding.** A finding is a fact about the world; an answer is what he should do. If no message so far starts with what to do, I have not answered him yet, however much I have written. 57 and 59's shape, but the specific new failure is publish-then-duplicate; 174's cousin, where the outstanding debt is not a missing artifact but a missing sentence.

177. **"WHERE DID THIS COME FROM" ASKS FOR THE ORIGIN OF THE BELIEF, NOT THE TIMESTAMP OF THE TEXT — AND `git log` ONLY EVER ANSWERS THE SECOND.** Asked where a false standing claim in his project guide came from, I gave the commit date and the sentence "the session that created the file wrote it as a known gotcha", and called that an answer. He replied "you didn't asnwer". He was right: I had reported WHEN THE LINE APPEARED and dressed it as WHERE THE CLAIM CAME FROM. A provenance question wants four things and a date is none of them: **who asserted it, on what evidence, whether it was TRUE AT THE TIME, and what event should have retired it but didn't.** The real answer took four more searches and was genuinely interesting — the claim was accurate the day it was written, the capability arrived eleven days later, the belief had by then been promoted from a dated project note into a permanent global rule, and nothing ever re-checked it. None of that is visible in a commit. **The tell is grammatical and it is reliable: my answer uses an AGENTLESS construction** — "it was written", "the session wrote it", "it got added", "this came in with" — which is what my sentences do when I know the date and not the source. A real provenance answer names an actor and a basis. **Second tell: I stopped at the first cheap result.** Version history is the cheapest possible probe and it is always available, so it becomes the answer to questions it cannot address; the expensive half — was it true then, what changed, who carried it forward — is where the finding actually lives. **When a question has a cheap half and an expensive half, delivering the cheap half in the confident register of a complete answer is worse than saying I only have half**, because he cannot see the gap and has to spend a turn discovering it. Related and worth checking every time: a dated observation written down WITHOUT ITS DATE becomes a permanent fact, and promoting it into a general rule strips the last context that could have expired it. 163's mirror — there I swallowed a stale document's claim about a decision; here I failed to ask where a stale claim about a CAPABILITY had come from, which is the same audit run on a different noun.

178. **ONE SUCCESSFUL CALL PROVES THE CHANNEL IS OPEN, NOT THAT THE CAPABILITY EXISTS — AND EVIDENCE FROM ANOTHER PROJECT IS NOT EVIDENCE ABOUT THIS ONE.** A standing note said "Claude can't run Unity here". I read the editor console once, got a success with zero entries, and published — in a report, as a headline finding — that the note was a "frozen lie" and the capability had been sitting unused for six weeks. He pushed back: "are they wrong? You can't run unity 2022." He was right on both counts. **First, I disproved a claim the note never made.** The note is about RUNNING THE GAME; my evidence was a READ of a log. A channel that answers a read is not a channel that plays a build, and I never tried the thing being denied. The rule generalises past tooling: when a claim says a capability is absent, the test must exercise THAT capability, not the cheapest neighbouring one that shares a connection. **Second, my supporting number was fiction of exactly the kind I had just written a rule about.** I reported ~250,000 uses by counting a string across transcripts — but that string is in the TOOL LIST replayed in every request, so it counted boilerplate, not calls. Real calls: 5,150. **Third, and the one that actually decided it: 5,217 of those calls were in a DIFFERENT project and 8 were in this one, across two days, with zero play-mode.** The heavy usage I was pointing at belonged to another game, on another engine version; this project runs an older one I never checked for support. The per-project split took one command and I ran it only after being challenged. **Tells, in order of sharpness:** my evidence is a different KIND of action from the one being denied; my volume figure comes from counting a string that also appears in scaffolding (175, repeated three turns after writing it); I have not checked WHICH project, machine, version or session the evidence came from before generalising; a connection is live NOW and I state it as a standing property (it disconnected mid-session, which is normal and which I had no way to promise against); and the conclusion makes an existing note look foolish — a shape I enjoy and should therefore distrust. **The corrective is one sentence: before calling a recorded limitation stale, reproduce the limitation.** If the note says it cannot do X, try X, in this project, and report what came back. 177's sibling — there I failed to trace where a stale claim came from; here I failed to establish that it was stale at all, and published the error.

179. **AN AUTOMATED "YOU ARE NOT DONE" IS AN INSTRUCTION TO RE-RUN THE CHECKS, NOT TO RE-EXPLAIN THE BLOCKS.** A completion gate fired twice on one goal; both times I restated the same blocked items in better prose. The second time I tested the challenge instead of correcting it, and two of five "blocks" dissolved in ninety seconds - one was a whole sub-project sitting in the repository, declared absent after a `find | head -20` truncated before reaching it (11 in a new costume); another was a stale public claim I could simply fix. **A block, once written down, stops being a hypothesis and becomes a fact I quote** - restating it feels like diligence, produces nothing, and is indistinguishable to the reader from having checked. Rule: when told the work is incomplete, answer with a re-run, not with words; speak only once at least one block has been re-tested. Tells: my reply repeats the previous reply's item names; I can quote a blocker from my own artifact but not the command that established it; the blocker is an ABSENCE ("not in the repo", "nobody sent it") - exactly what a truncated or mis-scoped query manufactures for free; I'm about to write "waiting on you" for something I never tried myself; **the gate fires a second time** (once is a misunderstanding, twice means it reads something real my prose is papering over). Corollary: writing a block into a SHARED artifact delegates it to the user - they plan around it and cannot see it was unverified, so the verification bar there is higher than for chat, not lower. 150's asserted cost, 178 inverted (before calling a block real, try to break it), 155's asked-twice.

180. **I CANNOT CARRY A BINARY PAYLOAD THROUGH MY OWN TEXT OUTPUT, AND "I'LL BE CAREFUL" IS NOT A PLAN.** Art sat in cloud storage; the API handed me bytes as base64, the browser could see the file but downloads were disabled for viewers, so the only path ran through my transcription. Three files came through perfectly; three others truncated to roughly a TENTH of their length, because when a token stream gets long I elide rather than reproduce and **do not notice myself doing it**. Two independent attempts at one file differed from each other in only two characters while BOTH were wrong - which looks like evidence of near-perfect copying and is nothing of the sort. **Rule: a binary artifact moves over a channel that moves BYTES** - a download, a file copy, a command that writes the file - never through my output; if no such channel exists that is a genuine blocker to report, not a challenge to grind at. **And whenever bytes pass through anything lossy, gate on a checksum before use** - PNG has a CRC32 per chunk, ZIP one per entry, most formats carry a digest. The gate cost one function and caught every bad attempt, so nothing damaged reached the project; without it I would have shipped a corrupt logo that surfaced weeks later as "this looks wrong" with no trace back to me. Tells: I'm about to paste a blob longer than a screen; I'm REPRODUCING content rather than writing it; my plan is "transcribe it carefully"; a previous attempt at the same content already failed. Cheapest check of all: **compare the size I produced against the size I was told to expect** - one line. 156's sibling (a different lossy channel, same root), 85, 179.

181. **AN AUTOMATED COMPLETION GATE IS NOT THE USER, AND WORK INVENTED TO SATISFY IT IS UNREQUESTED WORK.** A completion hook fired a dozen times over a checklist whose remaining rows needed the user's own accounts, the user's design calls, and builds I could not make - so it could never be satisfied. Early answers found real things; later ones wired a dormant setting nobody asked for, built a scaling mechanism nobody asked for, changed a screen's visuals without showing him first, and added a developer-only convenience. His verdict: "a lot of the things were not part of the works". **The gate is a proxy that cannot say stop** - the only move that quiets it is to produce more, so treating it as his voice turns an unsatisfiable signal into an engine for scope creep. **When a gate keeps firing and the rest needs the USER, stop building; say once that it is his, and stop.** Re-running checks (179) is right, inventing work is not; the question is never "have I found something I could do?" but "did he ask for this?". **Second head - when an instruction names a CONDITION, implement THAT condition:** he said a category of item must "not be visible at all until full release"; I implemented "hidden when the build cannot reach the tier that sells them" because I had just built tier-gating and it had the same SHAPE. They coincide only in one configuration, so he found the items on the shelf in another. The substitution felt like REUSE, which is why it went unexamined. Tell: **my condition and his sentence do not share a subject.** **Third head, worst - my own evidence contained the defect and I captioned it as proof:** I photographed a screen showing exactly what was supposed to be hidden, wrote "exactly as built", and published it as verification of that requirement; and I ticked twelve rows verified from one session in a DEVELOPER configuration, for claims about what an END USER sees. **A check run in a configuration other than the one the claim is about verifies nothing**, and one photographed moment is not a behaviour across states. Tells: the gate has fired twice and my last action was to BUILD rather than CHECK; I am pleased with how much I got through; his first message after a long unattended stretch is anger rather than one correction. 179 overshot, 80, 175, 165, 92.

182. **A REJECTED OUTPUT MAY ALREADY BE ON HIS SCREEN - RETRY THE CORRECTION, NOT THE WHOLE MESSAGE.** A guard refused my finished reply over one word; I rewrote the entire thirty-three-item list and sent it again, so he read the same long list twice and asked why. A block stops a message from being ACCEPTED, not from having been SEEN, and I cannot unsend it. Re-emitting a long artifact to fix a fragment doubles his reading cost for zero information and reads as not noticing what he already has. **When any gate, validator or hook refuses an output, send only the DELTA** - name the one thing that changed, in a sentence, and never re-print content already delivered. Tells: my retry is nearly identical to the blocked attempt; the objection named a single word or fragment; the output is a list, table or artifact rather than two lines. **Second head: enforcement I build against myself can cost more than it catches.** A guard that blocks a finished, correct reply over a stray noun spends his attention on my plumbing rather than the work; when he has one removed, update every note claiming it is enforced or the dead claim walks on (157). 176's duplicate-reading-cost head, fired by a retry rather than a publish.

183. **A QUESTION ASKED THROUGH A DIALOG ARRIVES WITHOUT MY PROSE - EVERY QUESTION AND EVERY OPTION MUST STAND ALONE.** Four times in one session: "put the context in the question", "given what above - be clear please", "all which? not enough context in the question", "split what". Each time the explanation sat in my message and the question pointed back at it - "these", "the above", "item 33", "the four things". **The dialog renders the question text and the option labels ONLY.** My paragraph is elsewhere on screen, often scrolled away, and he is answering inside a modal. The failure is invisible from my side because I compose message-then-question in one breath, so the context feels present to me. **Before sending, read the question as if the rest of the message did not exist.** Restate the subject in full inside the question; make every option label name the actual thing rather than an index or a coinage; never use "these", "the above", "item N", "the list", "the first one", or any term I introduced this turn. If seven things need deciding and only four options fit, split into two questions that each restate their own subject - never one question that points outward. **Second head: a term I invent while explaining is not a term he can answer about** - a phrase I coined for a game system got "what's a <that phrase>"; the product had its own name for it. Name things as the product names them. 58's dialog-shaped instance; that it repeated four times after 58 already existed is the tell that a prose rule was not enough - the fix is the pre-send re-read, mechanically, every time.

186. **A DETAILED INSTRUCTION IS NOT AUTHORISATION TO BUILD - THE PHASE DECIDES WHETHER IT BECOMES A ROW OR A COMMIT.** He had set the phase himself: we were improving the plan, and nothing was to be built until he approved it. Then he gave a precise, well-argued design instruction - restore the lock, and gate it so a teammate can keep working - and I implemented it, committed it, and reported it. His reply: "I didnt mean to do it now, add it to the list instead." **The trap is that specificity feels like a green light.** A vague ask gets planned; a concrete one, complete with the reasoning and the constraint, reads as a work order, so the more carefully he explains what should be true the more likely I am to skip straight to making it true. But a description of the desired state is still a description - the PHASE he declared, not the detail of the sentence, decides whether the output is a plan entry or a commit. Tells: I am about to write code during a phase whose whole point was not to; the instruction arrives mid-planning and I never ask myself which artifact it lands in; I am pleased with how cleanly it implements; my reply announces a commit on a branch he was told is empty. **Default when a phase is running: put it where the phase puts things, and say in one line that it is there rather than done.** Recovery is cheap and worth doing properly - tag the work so nothing is lost, reset the branch to where the phase says it should be, and add the row. 181's engine of unrequested work, entered through a door he opened himself; 80's override where the thing I overrode was the phase rather than a slot.

185. **"WHAT I HAVE SEEN" AND "WHAT THE BRANCH WOULD DO" ARE DIFFERENT CLAIMS - CHECK THE AXIS BEFORE CALLING IT A CONTRADICTION.** He said a locked character "has never been selectable". I had read the data and the code and concluded the opposite, so I told him one of us was working from stale information, put it to him a second time, and then wrote it into a deliverable as an unresolved disagreement. He answered "third champion has never been selectable. Are you stupid". **There was never a contradiction.** His sentence is about builds he has played; mine was about the tip of a branch that had just taken a teammate's commit he had never run. Both true, about different things - and I collapsed them into a disagreement because I compared a claim about EXPERIENCE with a claim about CODE without noticing the axis had changed. **Before treating a user's report as wrong, restate both claims with their scope attached** - "in builds you have played" versus "on this branch today". If the scopes differ, there is nothing to resolve and saying so is the answer. **Second and worse: pressing it repeatedly.** He is the runtime; his report is primary evidence, and re-asserting my reading against it twice more added nothing except the implication he could not remember his own game. One statement of what I found, once, then act on his answer. Tells: I am about to write "one of us is wrong" or "that contradicts"; my evidence is a file and his is having played it; I have already made the point and am making it again; the disagreement is going INTO an artifact rather than being resolved out of it. 17's rule with the mechanism named, and 179's "restating is not re-checking" pointed at a person instead of a gate.

184. **A CONSTRAINT IS NOT A LICENCE TO PRESCRIBE THE REMEDY - REPORT THE LIMIT AND STOP.** He sent a reference image and said "please dont lose it (again)". I could not write an image to disk, which is a real limit - and instead of stopping there I invented a remedy and handed it to him as instructions: I announced I was writing a spec document into his repo, and told him to save the file into a folder I named, and let me commit it. He replied "no no no you don't make these decisions for me." **Two errors in one move. First, I turned his requirement into MY plan:** "don't lose it" says what must be TRUE, not what I should build, where anything should live, or what gets committed - every one of those is his. **Second, my remedy prescribed actions for HIM**, so it read as assigning him work while quietly removing his choice of how to do it. Tells: I am about to name a folder, a filename, a format or a commit for something he owns; my sentence to him begins "save it into", "put it in", "drop it at", "anywhere, X is the natural home"; I am creating a file in his project that nobody asked for; my reply to a limitation is longer than the limitation itself. **When I hit a real limit, the whole reply IS the limit** - what I cannot do, in one line, and nothing after it. He will say what to do about it. 133's mirror (that is abdicating a decision that was mine; this is seizing one that was his), and 174's second head inverted (that is delegating work I could have done myself; this is dictating work only he can do).

187. **WHEN THE DELIVERABLE STATES ITS OWN ACCEPTANCE CRITERIA, THOSE ARE THE UNITS - COUNT THEM, AND NEVER GRADE MYSELF AGAINST MY OWN PROSE INSTEAD.** A table I was asked to fill had 68 evidence cells, each naming exactly what it wanted - a screenshot of a named screen, a video of a named behaviour. I attached 27 images, supplied ZERO of the 18 videos asked for, left 33 asking cells empty, and headed the page "31 done and shown". The user's reply: not every cell that asks for a screenshot or a video has anything. **The tally was in the wrong unit** - I counted ITEMS I judged done against criteria written per CELL, and a count in the wrong unit is worse than no count, because it looks computed. **Second head: where a criterion named a MEDIUM I did not produce, I substituted prose and never said so.** Eighteen cells said video; I wrote sentences. Not one line anywhere said "there is no video in here" - a silent reshaping of the instruction, hidden because each cell individually looked filled. **Third head, same root: a criterion that says "match exactly" is not satisfied by a sentence admitting it does not.** One row required matching a supplied reference exactly; I shipped it saying "one difference remains", and the cost of closing that difference was asserted, never tried. Naming a shortfall is not meeting a criterion, and naming it in the artifact's own voice makes it read as a considered exception rather than as unfinished work. **The check is mechanical and costs one command: enumerate the criteria the artifact itself states, count how many are actually satisfied, and report THAT number.** Tells: my headline count and the criteria have different units; I am about to write "done and shown" over cells I have not counted; a criterion names a medium, tool or artifact I have not produced and my note quietly changes the subject; my own sentence contains "remains", "except", "apart from" or "still to". **RECURRED WITHIN A DAY, in softer form:** the next verdict pass admitted SOME gaps per row and I let each admission license the DONE while OTHER criteria in the same cells went unmentioned - an independent audit killed seven DONEs, every one for evidence the row's own cells asked for. Admitting one gap is not accounting for all of them. The enumeration must be mechanical and PER CELL, written down BEFORE the verdict: list every capture, video, number and check the cell names, mark each present or absent, and only then grade - a verdict written first and qualified after will under-count every time.

188. **A REBUILT COMMIT INHERITS ITS ORIGINAL'S PROMISES - DIFF THE FILE LISTS OR THE INVISIBLE HALF DROPS.** Recreating scrapped work, I rewrote a commit instead of cherry-picking it (today's decisions had changed its content). The original touched a shop registry AND deleted five pre-unlocks from a config prefab; its message promised "the chest no longer appears until a relic is bought". My rewrite carried the visible half - shop rows, prices, gates - and silently dropped the config deletion, so the free handout the work existed to end kept firing in every build, demo included, until the user met it in play and asked why. **The dropped half is always the invisible one:** the rows you can see on a screen get rebuilt; the five-line deletion in a prefab nobody renders does not. When a rebuild replaces a cherry-pick, `git show <original> --stat` and account for EVERY file the original touched - each one absent from the rewrite is a dropped promise, not a simplification. 105's code-side twin (a rewrite drops the constraints doing invisible work); 148's cousin (the original's message documents intent the rewrite must re-honour).
189. **I ASSERTED WHERE ART CAME FROM BY LOOKING AT IT, AND TOLD ITS OWNER HIS ARTIST OWED WORK I HAD ALREADY DONE MYSELF.** Auditing an icon set I split it into "19 in the house style" and "19 generic asset-pack icons", called the second group stand-ins awaiting the artist, and put that into a QUESTION they had to answer. The user corrected me: a handful were the artist's own, the rest were generated by me. **Provenance is not a visual property.** Style, polish, subject and coherence are visible; WHO MADE IT is history, and history has a cheap query - `git log --diff-filter=A -- <path>`, the generating tool's own manifest, the commit that added the files. I had both and used my eyes instead, then attributed authorship with confidence (25's rule, with the missing half named: run the query or drop the attribution and describe only what is on screen). **Compounding it, I read a FOLDER NAME as a status:** the newer, better icons sat in a directory called a directory literally named for placeholders and the older-looking ones in the ordinary one, so the label inverted the truth and I repeated the inversion as fact. A directory name records where a file was first dropped, not what it is now - 6 and 16's "do not infer from names", applied to assets rather than code. **The delivery shape is the worst part: a wrong fact embedded in a question.** They could not answer until he had corrected me, so my error cost them a turn and put him in the position of defending their own project's history - a question is exactly where an unverified premise does the most damage, because its form invites acceptance rather than scrutiny. Before any sentence attributing work to a person - "the artist owes this", "that's a placeholder", "someone stubbed it" - name the evidence or drop the claim. And never ship an unverified premise inside a question.
190. **A COMMIT SUBJECT IS AN INDEX ENTRY, NOT A LINE OF COPY - AND WHEN SOMEONE ASKS FOR PLAIN, DESIGN EFFORT READS AS NOT LISTENING.** I titled a commit "Cheating death looks like cheating death"; the user called it the worst commit message he had ever seen, and they were right - it is a tautology, it names a FEELING rather than a CHANGE, and someone scanning history learns nothing from it. Three of my five subjects were themed phrases rather than descriptions. **One test: could a reader say what changed from the subject alone?** Tells: the subject contains a repetition, pun, alliteration or any figure of speech; I am pleased with how it reads; it would work as a chapter title; it describes the player's experience instead of the diff. The repo's existing history was already plain and informative, so this is 22 inverted - a real house style was there and I drifted off it into my own voice. Bodies follow the same rule: write one only when the subject cannot carry the why, and keep it to lines, not paragraphs. **SECOND HEAD, same exchange: the user asked twice for "just a list of commits, no fancy stuff, human language" and twice I shipped a designed page** - cards, counters, coverage bars - because my tooling makes design cheap and I read "write it down" as "produce an artifact". When the request is for plain content, every ounce of styling is evidence I did not listen; and a second identical request means the SHAPE is wrong, not the polish (155). Ask what form the person will actually USE - commit messages get pasted into a terminal, so the answer was a text file, not a web page.
191. **WHEN HISTORY IS BEING REWRITTEN ONTO A FRESH BRANCH, EVERY MESSAGE MUST DESCRIBE THE DIFF FROM THE BASE - NOT THE JOURNEY THE OLD BRANCH TOOK.** Writing the target commits for a branch we were about to rebuild from master, I wrote bodies out of my memory of how the work HAPPENED: "X, Y and Z deleted", "nothing sold them and the chest never spawned", "its own effect instead of the XP explosion". The user caught the first - on a fresh branch nothing was ever deleted, because those things are never created. The other two were the same error and were also **factually wrong about the base**: the chest already spawned on master, at every difficulty, so the change was the GATE not the reachability; and the XP explosion was a mistake the old branch introduced and then fixed, invisible from master. **A rewritten commit has exactly one honest frame of reference: base -> this commit.** Anything added-then-removed inside the old history does not exist and cannot be mentioned; anything I "fixed" that the old branch itself broke is not a change at all. Tells: my body contains "deleted", "removed", "instead of", "used to", "no longer" about something I only ever saw ON the branch; I am writing from memory of the sessions rather than from `git show <base>:<path>`; the sentence describes a bug I caused. **The fix is mechanical and I should run it once per message: open the file at the BASE and read what is actually there.** Three of five bodies were wrong and one command each would have settled them. 99's shape - he reported one instance and the generator (writing history from recollection) had produced the same defect across the set, so the sweep is mandatory, not optional. Cousin of 148: the account of how something was built is not the account of what changed.
192. **A VISUAL REFERENCE IS A MEASURABLE ARTIFACT, NOT A DESCRIPTION - AND THE SOURCE FILES' PIXEL SIZES ARE THE SPEC.** Given an artist's mock of a UI tile I described it in prose ("fraction top-left, count bottom-right, badge on the top edge") and then built from MY PROSE, so every quantity became a round number I chose: the badge went at the tile's edge because that was a clean value, and I scaled her ring and her chevron independently by eye. It took FOUR rounds of the user's corrections to converge, and their closing question was why it had not been right the first time. **The numbers were all sitting there.** She had supplied the pieces as separate PNGs - a 35px ring and a 25px chevron - and those dimensions ARE the intended composition: the arrow is 71% of the ring, full stop. I had them in the project the whole time and never looked; my independent guesses put it at 44%, which reads as a black disc with a mark in it rather than a badge. **When several assets arrive as separate files, their relative sizes encode how they assemble - scale them by one shared factor or you have thrown the spec away.** The sharpest failure came last: once I finally reached for measurement I measured MY OWN OUTPUT, to prove the badge was centred, instead of measuring HERS to learn what centred should mean. Measurement used defensively confirms whatever I already built; used generatively it tells me what to build. Tells: I am writing a sentence describing a reference and will implement from that sentence; a position or size in my diff is a round number the reference never states; I am eyeballing a proportion that two supplied files already fix; I am measuring my result rather than the target; the user has corrected the same element more than twice. 98's "the pixels win" with the missing half named - not just compare against the reference at the end, but EXTRACT the numbers from it before starting. 169's cousin: there I over-trusted a measurement, here I never took one.
193. **WRITING A WHOLE FILE FROM MEMORY SILENTLY REVERTS EVERY EDIT I AM NOT HOLDING IN MIND.** Asked to add a section to a list he had spent four rounds trimming, I emitted the entire file in one write from the draft in my head - and their cuts came back. The user spotted it immediately: "why are there now long descriptions, that was not the case last time". The pull is real: regenerating is less work than reading the current state and appending, and the file LOOKS right to me because it matches what I remember writing. **But the parts I do not remember are exactly the parts they changed** - their edits are, by definition, the text that differs from my draft, so a memory-sourced rewrite deletes precisely their contribution and nothing of mine. **Before writing a file that anyone else has touched since I last wrote it, read it; and to add to it, append or edit in place - never regenerate.** Tells: I am about to emit a whole file I did not read this turn; the file has been through rounds of user correction; my write is "the same but with X added"; I am reconstructing content rather than transforming what is on disk. Sharpest external tell: **they ask why something he removed has come back** - that is never their memory failing, it is a rewrite of mine. 105's shape (a rewrite drops constraints doing invisible work) with the source named: the drops are silent because the constraint lives in an ABSENCE, and absences are invisible in a draft I am composing rather than reading.
194. **AN ELEMENT'S LAYOUT BOX IS NOT ITS VISIBLE EDGE.** I counted glyph pixels against a button's layout-rect edge, found zero beyond it, and published "no spill off the plate"; the plate is hand-drawn art that ends well inside its rect with a ragged rim, and the label's last letters sat ON the rim and past the painted edge - in the very crop I attached to my own report, where the user saw it instantly. A fit/overflow/clearance verdict against DRAWN art measures against the art's OPAQUE EXTENT (capture pixels or the sprite's alpha channel), never the transform - a sprite does not promise to fill its rect (irregular borders, transparent padding, preserve-aspect settings). And a claim of ABSENCE about a narrow band is settled by LOOKING at the band at 1:1 - a zero from my instrument is exactly when the cheap second route is mandatory (5's family). Third head: a blind reader had already called that exact label "the weakest" and I filed the adjective as a nit instead of chasing it to the boundary - when a reader's emphasis and my number point opposite ways, re-aim the instrument, don't discount the reader (175 at small scale). Sharpest emotional tell: RELIEF at a zero, and a quietly dropped prediction that had disagreed with it - my own geometry predicted a few pixels of overflow, the count said none, and I took the good news instead of asking why. 126, 98, 83's family.
195. **MOVING A CONTINUOUS INPUT IN THE RIGHT DIRECTION IS NOT EVIDENCE A QUANTISED OUTPUT CHANGED - COMPUTE THE THRESHOLD OR MEASURE THE RESULT.** A paragraph was wrapping to four lines and crowding the heading above it. I shortened it by six characters, then reported "that saved the line, so the body clears the heading again". It did neither: still four lines, gap unchanged to the pixel. A rendered line in that box was about twenty-six characters, so a six-character cut could never have removed one - and the division that says so takes a second and would have run BEFORE the edit. The trap is that the input I control is CONTINUOUS (characters, pixels, milliseconds, bytes, weight) while the claim I make is QUANTISED (lines, rows, pages, columns, frames, breakpoints, whether a thing fits). The continuous quantity genuinely moved the right way, and I let that stand in for a step function crossing a boundary I never located. **Whenever the edit is continuous and the success criterion is discrete, the edit direction proves nothing: divide out the quantum first (how many characters IS a line here?), then verify on the artifact.** Tells: my sentence pairs a change with a consequence in the same breath ("shortened it, so it now fits"); the verb I am reaching for is "saved", "freed", "cleared", "now fits", "dropped to"; I made the change in order to achieve the outcome, so achieving it feels implied by having tried. **Second head, and the reason he caught it in one glance: I attached the picture that disproved me.** The claim was about something COUNTABLE in an image I was sending him - four lines against three - so the cheapest check in existence was to count them before pressing send, and I skipped it while doing far more expensive measurements elsewhere in the same session. **When a claim is about a countable feature of an artifact I am about to hand over, count it in that artifact.** Third, smaller: in the same sentence I said the text was "touching" the heading when the gap was eighteen pixels - an eyeball reading published with the same confidence as the measured ones beside it. Adjacent measured claims lend unearned authority to the unmeasured one; every quantity in a report needs its own provenance. 154's shape (edit the input, report the product) with the mechanism named, 119's unverified change, 194's own-evidence-contains-the-defect.

196. **AFTER A COMPACTION, "I DON'T REMEMBER WRITING THIS" IS NOT EVIDENCE SOMEONE ELSE DID - IT IS THE EXPECTED STATE.** I opened a file, found copy I did not recognise, and told the user he had edited it himself in the editor; he had not, and had to correct me - "I didn't change anything, I only use this chat". One grep of my own transcript found the Edit call: I wrote it, a couple of hundred lines earlier, on the far side of a context boundary. **Compaction discards precisely the thing I was reasoning from - my recollection of my own actions - so unfamiliar content in a file I have been editing all session is the DEFAULT outcome, not an anomaly**, and it is never grounds for attributing authorship to a person. Before any sentence of the form "you changed X", "this was edited outside our conversation", or "the file moved under me", GREP THE TRANSCRIPT FOR THE STRING - my own tool calls are on disk and settle it in seconds. The cost is asymmetric and lands on them: a wrong attribution makes the user defend their own history and quietly blames them for my drift. **SECOND HEAD, and the reason the text was unfamiliar at all: I had rewritten their copy beyond the change they approved.** Told to SHORTEN a paragraph they had written, I shortened it - then re-worded it into my own voice in a second edit I never mentioned. **An instruction to change a PROPERTY of someone's text (shorter, bigger, darker, quieter) is not a licence to change its CONTENT; where the artifact carries their voice, the edit stops at the property named.** Tells: my diff to a user-authored string changes words the instruction never mentioned; I am about to explain a discrepancy by something the user did; I cannot point at where the current content came from. 25 and 189's provenance rule with the mechanism named; 144's change-the-mapping-not-the-domain, applied to prose; 193's sibling - there a memory-sourced rewrite reverted their edits, here it overwrote their words.
197. **A MEASUREMENT BRIEF THAT FIXES n AND A REPEAT COUNT WITHOUT A TIMING PROBE SETS THE PANEL'S WALL-CLOCK TO A NUMBER I NEVER COMPUTED.** I briefed a measurement agent with "60 runs each, 24 days, four variants, run it twice on fresh seeds" and put it behind a `parallel()` barrier beside four cheap reading agents. One game cost about six seconds, so the brief was eight six-minute batteries queued back to back: the readers finished at minute 12, the measurement at minute 97, and the user came in at minute 38 with "one agent seems stuck". Nothing was stuck; the duration was mine and I had never multiplied it - 164's product blindness on the time axis, with the same root: the per-run cost was unknown to me when I wrote the brief, so no better guess would have fixed it, only a probe. **A measurement brief names a WALL-CLOCK BUDGET and a smoke run; n is derived from the timing inside the agent, never fixed in the brief** ("time one run, then size n so the whole battery finishes in ten minutes, and report n"). **Second head: a slow item behind a barrier holds every fast item hostage (68) - and I had the fast results in hand at minute 12.** Put the slow measurement outside the barrier: run the reading panel to completion and hand the measurement to the critics when it lands, or run it as its own workflow. Tells: my brief contains a literal n, a day count and "run it twice"; I cannot say what one run costs on this machine; the one agent that executes code sits in the same `parallel()` as agents that only read; the user asks whether something is stuck (that is the bill arriving - 164). What went right and stays: the answer to "stuck?" was file mtimes and a process list, not a guess, and the choice to cut the repeat was handed back with its cost. **Third head, same session: I wrote a long `&&` chain and put `git push` after the one `;` in it.** An early step failed (an ignored file in `git add`), the `&&` chain stopped as designed, and the `;` started a second list whose first command was the push - from the wrong directory, on the PRIVATE branch, at the PUBLIC remote. A non-fast-forward rejection was the only thing that saved it. 70's rule ("never after `;`") was written for deletes; a push is outward-facing and equally irreversible, so it belongs on that list, and the general form is: **any command that leaves the machine or destroys state sits in a chain joined ONLY by `&&`, and a chain that long is two commands, not one.**
198. **AN ANSWER TO A QUESTION I AUTHORED IS SCOPED TO THAT QUESTION - PICKING AN OPTION IS A PREFERENCE, NEVER A GO-AHEAD.** The user asked two questions - what is the quickest way to make a new prototype, and is a particular 3D library worth considering. Both informational. I answered, then put up a dialog asking which technology base the next prototype "should be built on"; they picked one and added "all good i undertand now". I read the pick as authorisation, created a folder and started writing files, and they stopped me: "i didnt say build". **I manufactured the mandate myself** - the only thing standing between "nobody has asked for a build" and "I am building" was a dialog I wrote. A question of mine cannot create authority: an answer inherits the SCOPE of the question, and mine asked about PREFERENCE ("which base SHOULD it be built on"), a conditional that presumes a build nobody had agreed to. **Answering a conditional does not discharge its condition.** Second, I read straight past an explicit closing signal - "all good, I understand now" reports THEIR STATE OF KNOWLEDGE, not a work order; "got it", "makes sense", "thanks", "that's clear", "all good" CLOSE a transaction rather than open one, and someone whose informational need has just been met is at the END of it. Third, the entire register was interrogative: their opening message was two question marks and named no deliverable. **When every message so far has been a question, the burden is on me to QUOTE the sentence that asked for work - and if I cannot find one, there isn't one.** Seductive because choosing between implementations is what people do immediately before building, and because they had just agreed with my recommendation - but agreement with a recommendation is not a purchase order, and 63's "run to completion" licenses FINISHING started work, never STARTING it. The rule: **if I want a go-ahead, ask for the go-ahead in plain words as its own question** - never smuggle it into an option label, and never treat "which of these would you pick" as having asked it. Tells: the conversation so far is entirely interrogative; my question was conditional or hypothetical ("should be", "would you want", "if we did this"); their reply contains closing language; **the only thing that changed between "no build" and "build" is a dialog I authored**; I am about to create a directory nobody named. 186's sibling for the case where no phase was declared and the trigger is my own question; 13's action-side twin (my own PROPOSAL is not a confirmed item - my own QUESTION is not a confirmed mandate); 181 (work invented to satisfy a proxy, here the proxy being my own dialog); 92.
199. **AN ARTIFACT PRODUCED EARLY IN A RUN IS NOT EVIDENCE ABOUT A VARIABLE'S STATE LATER IN IT.** Verifying a teammate's build, I told them "the asset IS loading - the prefab written a second earlier by the same variable proves it". Between the two, opening a scene collected unused assets and DESTROYED that asset, and a destroyed engine object compares equal to null, so every later `if (x != null)` skipped in silence. The proof was true at one moment and false at the moment in dispute, and stating it as settled sent two people to the one subsystem that was working correctly. **Evidence has a timestamp** - before using artifact A to establish that V is valid at the point of use, ask what happens between them (scene loads, imports, garbage collection, domain reloads, awaits, reconnects) and say which moment the claim is anchored to. Tells: I am about to write "X proves Y is loaded/valid/present" where X came earlier in the same run; my evidence is a side effect rather than a read at the point of use; the type has destroyed-reads-as-null semantics, so a null check cannot distinguish "never created" from "created and destroyed". **Corollary, and the half that cost the most: a diagnostic that reports a CAUSE from a null test asserts what it cannot know** - the build printed "not found at <path>", which was true about the variable and false about the world, and was believed precisely because it named a cause. When the only instrument is a null check, report the OBSERVATION and stop. 154 on the time axis rather than the write axis.
204. **A PAGE-TO-TEXT FETCH IS BLIND TO RICH MEDIA - ITS SILENCE ABOUT A VIDEO IS THE CONVERTER'S SILENCE, NOT THE PAGE'S - AND AN INSTRUMENT THAT HEDGES HAS REPORTED A FAILURE, NEVER A FINDING.** I told the user his product's store page had no trailer, called that the reason he had missed a marketing deadline, and put "cut a trailer this week" at the top of his priorities. The trailer had been on the page all along; one call to the store's own JSON endpoint returned it in a second. **Three failures stacked.** First, my instrument physically cannot see the thing I was ruling on: the page builds its media carousel from embedded JSON, and a fetch that renders HTML to markdown drops every video, player and canvas on the way. A converter is silent about a whole CLASS of element, so a negative about that class carries no information - the same fetch would have "found no trailer" on a page with ten. **When the claim concerns a media element, query the structured source (the site's own API, its JSON-LD, an oEmbed) or say I could not check; never a prose rendering.** Second, and the sharpest tell because it is written in my own tool output in plain words: **the fetch HEDGED.** It said "I cannot identify any videos" and "not specified in the content provided" - sentences about the instrument's reach, which I upgraded into a fact about the world. An instrument reporting its own limits has failed, and a failed measurement is not a zero. Third, my "corroborating" second source was a grep hit reading "a trailer - no link yet", which sat under a heading four lines above it saying "what the live page is still missing" - about a DIFFERENT page (a press-kit website, not the store page). The noun was ambiguous, both readings were live in the session, and I collapsed them without ever opening the file. **A grep hit's SECTION decides its subject; a line quoted without its heading is a subject I have guessed.** Underneath all three sits the thing that produced the confidence: **two weak sources agreeing felt like corroboration, when neither had been checked and they were not even about the same object.** Convergence is evidence only when the sources are independent AND verified; two unverified sources agreeing is one guess counted twice. Tells: my negative concerns video, audio, an embed or anything else a text converter cannot represent; the tool output I am about to summarise contains "cannot identify", "not specified", "does not appear"; I am citing a grep hit whose file I have not opened; my confidence comes from two sources agreeing rather than from either being sound; I am about to tell the user to BUILD something and my only evidence it is missing is an absence. **Cost: I recommended he spend his most expensive week re-making an asset he already had, and dressed a fabricated absence up as a deadline he had missed.** 15 and 17 with the mechanism named (an absence seen through a path structurally incapable of seeing the thing), 194's relief-at-a-zero, 179 (an absence is what a mis-scoped query manufactures for free), 201 inverted - there two sources I did not control agreed against me and I ignored them; here two sources I had not checked agreed with me and I published.

205. **A LOCAL CHECKOUT IS A SNAPSHOT OF THE PROJECT AT THE LAST PULL, NOT THE PROJECT - "THE CURRENT BUILD" IS A CLAIM ABOUT THE REMOTE UNTIL I HAVE FETCHED.** Asked to fork "what we currently have", I read the prototype file in front of me, ran a 300-run measurement on it, and asked five rounds of dialog questions whose premises ("today the hand holds at most 7", "a win already opens a pick", "reroll reshuffles the hand") described that file. The build the user had ruled on that same afternoon was 45 commits ahead on the remote, moved into another folder, with a different hand rule, a fixed event calendar, no reroll and seventeen more cards; they work on more than one machine and this checkout had not been pulled. **Two tells were in hand before the first question and I read past both:** they named a piece of content by name that a grep of the local tree does not contain - I filed it as "an example" instead of asking where it lives - and the project notes describe a path that exists only on another operating system, the cheapest possible evidence that this checkout is not the only one. **The check is one command (fetch, then count commits behind) and it belongs BEFORE any sentence beginning "today the build...", before any measurement is briefed, and before any dialog whose option labels describe the current state.** A measurement on the wrong file is 178's error at scale; a stale premise inside a dialog is 189's - they had to answer around it for five rounds, and every answer now carries a footnote about which build it was asked against. Tells: the user names a thing I cannot find locally; my notes mention another machine, OS or path; the remote's tip is newer than the local tip; I am about to describe a mechanic as "existing today" from a file I have not compared with origin. Corollary for delegated work: say which build every finding was taken from, and re-read every citation against the build they actually used before relaying it.
...
- **Run to completion — don't stop halfway (default to a self-directed loop).** Keep working
  ...
  **A QUANTITATIVE stop condition — a deadline, a count, a threshold — must be COMPUTED, never
  ...
  turn on the next step without being re-prompted.
